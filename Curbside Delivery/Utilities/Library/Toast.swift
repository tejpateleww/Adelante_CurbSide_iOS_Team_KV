...
let AlertCheckImg = #imageLiteral(resourceName: "AlertCheckImg")
let AlertCancelImg = #imageLiteral(resourceName: "AlertCancelImg")
let AlertInfoImg = #imageLiteral(resourceName: "AlertInfoImg")
//
//class Toast {
//
...
class Toast {
    ...
    static func show(title: String = "" ,delay :Double = 1.5,  message: String, state : MessageAlertState ,  completion: (() -> Void)? = nil) {
        ...
        switch state {
        case .success:
            toastContainer.backgroundColor = UIColor(hexString: "61A117")
            statusImage.image = AlertCheckImg
        case .failure:
            toastContainer.backgroundColor = UIColor(hexString: "F9422F")
            statusImage.image = AlertCancelImg
        case .info:
            toastContainer.backgroundColor = UIColor(hexString: "FAA421")
            statusImage.image = AlertInfoImg
        case .theme:
            toastContainer.backgroundColor =  UIColor(hexString: "E34A25")
            statusImage.image = AlertInfoImg
        }
        ...
        if title != "" {
            ...
            let messagetoPrint = NSMutableAttributedString().normal(message, fontSize: 15.0 , fontColor: UIColor.white)
...
extension NSMutableAttributedString {
 var fontSize:CGFloat { return 18 }
    var normalFont:UIFont { return FontBook.regular.font(ofSize: fontSize) }
    func normal(_ value:String, fontSize : CGFloat , fontColor : UIColor = .white) -> NSMutableAttributedString {
        let attributes:[NSAttributedString.Key : Any] = [.font : FontBook.regular.font(ofSize: fontSize),.foregroundColor : fontColor]
        self.append(NSAttributedString(string: value, attributes:attributes))
        return self
    }
    func bold(_ value:String , fontSize : CGFloat,  fontColor : UIColor = .white) -> NSMutableAttributedString {
        let attributes:[NSAttributedString.Key : Any] = [.font : FontBook.bold.font(ofSize: fontSize),.foregroundColor : fontColor]
        self.append(NSAttributedString(string: value, attributes:attributes))
        return self
    }
}
