...
class submitButton: UIButton {
    ...
    override func awakeFromNib() {
        ...
        self.titleLabel?.font = FontBook.bold.font(ofSize: 18)
        ...
        if isAppOrangeBg {
            ...
            self.backgroundColor = .clear
            self.titleLabel?.font = FontBook.regular.font(ofSize: 14)
            self.setTitleColor(colors.appOrangeColor.value, for: .normal)
        } else if isForgotPass {
            self.backgroundColor = .clear
            self.titleLabel?.font = FontBook.regular.font(ofSize: 12)
            self.setTitleColor(colors.forgotpassGreyColor.value, for: .normal)
        } else if isShareOrderDetails {
            self.backgroundColor = .clear
            self.titleLabel?.font = FontBook.bold.font(ofSize: 12)
        }
        if isUnderline {
            self.setunderlineWithUIColor(title: self.titleLabel?.text ?? "", color: underlineColor ?? UIColor.clear , font: (self.titleLabel?.font)!)
        }
...
class PayButton:UIButton{
    override func awakeFromNib() {
        ...
        self.backgroundColor = colors.appOrangeColor.value
        self.setTitleColor(colors.white.value, for: .normal)
        self.titleLabel?.font = FontBook.bold.font(ofSize: 17)
    }
}
...
class myOrdersBtn: UIButton {
    ...
    override func awakeFromNib() {
        ...
        self.titleLabel?.font = FontBook.bold.font(ofSize: 16)
    }
}
    ...
    override func awakeFromNib() {
        if isChangeLocation {
            let yourAttributes: [NSAttributedString.Key: Any] = [
                .font: FontBook.bold.font(ofSize: 14),
            ...
            let yourAttributes: [NSAttributedString.Key: Any] = [
                .font: FontBook.regular.font(ofSize: 14),
                .foregroundColor: UIColor(hexString: "#E34A25"),
                .underlineStyle: NSUnderlineStyle.single.rawValue]
            let attributeString = NSMutableAttributedString(string: (self.titleLabel?.text ?? ""),
                                                            attributes: yourAttributes)
...
class btnApply : UIButton {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        if isApply{
            self.titleLabel?.font = FontBook.bold.font(ofSize: 15)
...
class btnOTP: UIButton {
    ...
    override func awakeFromNib() {
        ...
        if isResendOTP {
            ...
            self.titleLabel?.font = FontBook.bold.font(ofSize: 15)
            if isUnderline {
                self.setunderline(title: self.titleLabel?.text ?? "", color: colors.appOrangeColor, font: self.titleLabel?.font ?? FontBook.bold.font(ofSize: 15))
            }
        } else if isCreateAccount {
            self.setTitleColor(colors.appOrangeColor.value, for: .normal)
            self.titleLabel?.font = FontBook.black.font(ofSize: 14)
            self.setTitle("login_btnCreateAccount".Localized(), for: .normal)
            if isUnderline {
                self.setunderline(title: self.titleLabel?.text ?? "", color: colors.appOrangeColor, font: self.titleLabel?.font ?? FontBook.bold.font(ofSize: 14))
            }
        }  else {
            self.isUserInteractionEnabled = false
            self.setTitleColor(colors.forgotpassGreyColor.value, for: .normal)
            self.titleLabel?.font = FontBook.regular.font(ofSize: 15)
        }
    }
}
