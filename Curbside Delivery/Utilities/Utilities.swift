...
class Utilities:NSObject{
    ...
    static func displayAlert( title: String, message: String, completion:(( _ index: Int) -> Void)?, otherTitles: String? ...) {
        ...
        DispatchQueue.main.async {
            AppDelegate.current.window?.rootViewController!.present(alert, animated: true, completion: nil)
        }
    }
    ...
    static func showAlertWithTitleFromVC(vc:UIViewController, title:String?, message:String?, buttons:[String], isOkRed : Bool, completion:((_ index:Int) -> Void)!) -> Void{
        ...
        for index in 0..<buttons.count {
            if isOkRed{
                if buttons[index].lowercased() == StringConsts.Ok || buttons[index].lowercased() == StringConsts.Yes{
                    style = .destructive
                }else{
                    style = .default
                }
    ...
    static func displayAlert(_ message: String) {
        Utilities.displayAlert(AppName, message: message, completion: nil)
    }
    ...
    class func ShowAlert(OfMessage : String) {
        let alert = UIAlertController(title: AppName, message: OfMessage, preferredStyle: UIAlertController.Style.alert)
        ...
        if let res = param as? String {
            ...
            if let dictIndxZero = resAry.firstObject as? NSDictionary {
                if let message = dictIndxZero.object(forKey: "message") as? String {
                    Utilities.showAlert(AppName, message: message, vc: vc)
                }
    ...
    class func getMessageFromApiResponse(param: Any) -> String {
        ...
        return StringConsts.UrlConstant_SomethingWentWrong
    }
